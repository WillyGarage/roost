...
$namesBefore = @(Get-RoostDesktopNames)
$countBefore = $namesBefore.Count
Write-Host "desktops before: $countBefore"

$created = 'Roost Manage Test'
$renamed = 'Roost Renamed Test'
...
$afterCreate = @(Get-RoostDesktopNames)
...
$createdAt = Get-RoostDesktopPosition $created
...
$afterRename = @(Get-RoostDesktopNames)
...
$before = Get-RoostDesktopPosition $renamed
...
$destination = @(Get-RoostDesktopNames) | Where-Object { $_ -ne $renamed } | Select-Object -Last 1
...
$afterDelete = @(Get-RoostDesktopNames)
