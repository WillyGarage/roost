...
$script:RoostRegRoot = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VirtualDesktops'

<#
Reads Roost's config.

Roost writes its config as JSON documented with // comments. Windows PowerShell 5.1's
...
#>
function Get-RoostConfig {
    $path = Join-Path $env:APPDATA 'Roost\config.json'

    if (-not (Test-Path $path)) {
        throw "Roost config not found at $path. Has the app run yet?"
    }
...
#>
function Get-RoostDesktopGuids {
    $blob = (Get-ItemProperty $script:RoostRegRoot -Name VirtualDesktopIDs).VirtualDesktopIDs
...
function Get-RoostDesktopNames {
    Get-RoostDesktopGuids | ForEach-Object {
        $n = (Get-ItemProperty (Join-Path $script:RoostRegRoot "Desktops\{$_}") `
                -Name Name -ErrorAction SilentlyContinue).Name
        if ($n) { $n } else { '(unnamed)' }
    }
}

function Get-RoostDesktopCount { (Get-RoostDesktopGuids | Measure-Object).Count }
...
#>
function Get-RoostWindowMap([string]$RepoRoot) {
    $dotnet = Join-Path $env:LOCALAPPDATA 'Microsoft\dotnet\dotnet.exe'
    if (-not (Test-Path $dotnet)) { $dotnet = 'dotnet' }

    $out = & $dotnet run --project (Join-Path $RepoRoot 'spike\Roost.Spike') --no-build -- `
        --list --windows 2>&1
...
#>
function Get-RoostCurrentDesktopName([string]$RepoRoot) {
    $dotnet = Join-Path $env:LOCALAPPDATA 'Microsoft\dotnet\dotnet.exe'
    if (-not (Test-Path $dotnet)) { $dotnet = 'dotnet' }
...
#>
function Get-RoostOtherDesktopName([string]$Exclude) {
    $other = @(Get-RoostDesktopNames) | Where-Object { $_ -ne $Exclude } | Select-Object -First 1
...
function Get-RoostDesktopPosition([string]$name) {
    $all = @(Get-RoostDesktopNames)
